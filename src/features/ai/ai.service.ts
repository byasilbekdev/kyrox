...
import { CACHE_TTL, cacheKey, getCached, setCached } from "../../redis/redis.js";
import { callOpenRouter } from "./openrouter.client.js";
import { streamOpenRouter } from "./openrouter-stream.client.js";
import type { AiCompletionResult } from "./ai.types.js";

// Cache identical (prompt, model, last-message) triples briefly to
// absorb accidental double-sends / retries without burning AI credits.
function buildResponseCacheKey(ownerId: string, conversationId: string, userMessage: string) {
  return cacheKey("ai-response", ownerId, conversationId, Buffer.from(userMessage).toString("base64").slice(0, 64));
}
...
export const aiService = {
  async generateReply(
    ...
    const cacheKeyStr = buildResponseCacheKey(ownerId, conversation.conversation.id, userMessage);
    ...
    const context = await conversationService.buildContextWindow(conversation.conversation.id);
    ...
    await conversationService.maybeCompactConversation(
      ...
      async (messages, existingSummary) => this.summarizeMessages(messages, existingSummary, settings.aiModel)
    );
  ...
  async summarizeMessages(
    ...
    const result = await callOpenRouter({
      ...
      temperature: 0.3,
      maxTokens: 300,
      systemPrompt: "Siz suhbat xulosalovchisiz. Faqat qisqa, faktik xulosa yozing.",
      summary: null,
